...
type KBFSOps interface {
	...
	GetTLFCryptKeys(ctx context.Context, tlfHandle *TlfHandle) (
		keys []kbfscrypto.TLFCryptKey, id tlf.TlfID, err error)

	// GetTLFID gets the tlf.TlfID for tlfHandle.
	GetTLFID(ctx context.Context, tlfHandle *TlfHandle) (tlf.TlfID, error)
	...
	Rekey(ctx context.Context, id tlf.TlfID) error
...
type KeyMetadata interface {
	// tlf.TlfID returns the ID of the TLF for which this object holds
	// key info.
	TlfID() tlf.TlfID
...
type MDCache interface {
	// Get gets the metadata object associated with the given tlf.TlfID,
	// revision number, and branch ID (NullBranchID for merged MD).
	Get(tlf tlf.TlfID, rev MetadataRevision, bid BranchID) (ImmutableRootMetadata, error)
	...
	Delete(tlf tlf.TlfID, rev MetadataRevision, bid BranchID)
}

// KeyCache handles caching for both TLFCryptKeys and BlockCryptKeys.
type KeyCache interface {
	// GetTLFCryptKey gets the crypt key for the given TLF.
	GetTLFCryptKey(tlf.TlfID, KeyGen) (kbfscrypto.TLFCryptKey, error)
	// PutTLFCryptKey stores the crypt key for the given TLF.
	PutTLFCryptKey(tlf.TlfID, KeyGen, kbfscrypto.TLFCryptKey) error
}
...
type BlockCache interface {
	...
	// a nil error.
	CheckForKnownPtr(tlf tlf.TlfID, block *FileBlock) (BlockPointer, error)
	...
	Put(ptr BlockPointer, tlf tlf.TlfID, block Block,
	...
	// won't be reused.
	DeleteTransient(ptr BlockPointer, tlf tlf.TlfID) error
	...
	DeleteKnownPtr(tlf tlf.TlfID, block *FileBlock) error
}
...
type DirtyBlockCache interface {
	...
	Get(tlfID tlf.TlfID, ptr BlockPointer, branch BranchName) (Block, error)
	// Put stores a dirty block currently identified by the
	// given block pointer and branch name.
	Put(tlfID tlf.TlfID, ptr BlockPointer, branch BranchName, block Block) error
	...
	RequestPermissionToDirty(ctx context.Context, tlfID tlf.TlfID,
	...
	// block.
	UpdateUnsyncedBytes(tlfID tlf.TlfID, newUnsyncedBytes int64, wasSyncing bool)
	...
	// never be called).
	UpdateSyncingBytes(tlfID tlf.TlfID, size int64)
	...
	BlockSyncFinished(tlfID tlf.TlfID, size int64)
...
type cryptoPure interface {
	...
	MakeRandomTlfID(isPublic bool) (tlf.TlfID, error)
...
type MDOps interface {
	...
	GetForHandle(
		...
		tlf.TlfID, ImmutableRootMetadata, error)
	...
	GetForTLF(ctx context.Context, id tlf.TlfID) (ImmutableRootMetadata, error)

	// GetUnmergedForTLF is the same as the above but for unmerged
	// metadata.
	GetUnmergedForTLF(ctx context.Context, id tlf.TlfID, bid BranchID) (
		ImmutableRootMetadata, error)

	// GetRange returns a range of metadata objects corresponding to
	// the passed revision numbers (inclusive).
	GetRange(ctx context.Context, id tlf.TlfID, start, stop MetadataRevision) (
		[]ImmutableRootMetadata, error)

	// GetUnmergedRange is the same as the above but for unmerged
	// metadata history (inclusive).
	GetUnmergedRange(ctx context.Context, id tlf.TlfID, bid BranchID,
	...
	// branch.
	PruneBranch(ctx context.Context, id tlf.TlfID, bid BranchID) error
	...
	GetLatestHandleForTLF(ctx context.Context, id tlf.TlfID) (
		BareTlfHandle, error)
}
...
type BlockOps interface {
	...
	Delete(ctx context.Context, tlfID tlf.TlfID, ptrs []BlockPointer) (
	...
	// than folder writers.
	Archive(ctx context.Context, tlfID tlf.TlfID, ptrs []BlockPointer) error
}
...
type MDServer interface {
	...
	GetForHandle(ctx context.Context, handle BareTlfHandle,
		mStatus MergeStatus) (tlf.TlfID, *RootMetadataSigned, error)

	// GetForTLF returns the current (signed/encrypted) metadata object
	// corresponding to the given top-level folder, if the logged-in
	// user has read permission on the folder.
	GetForTLF(ctx context.Context, id tlf.TlfID, bid BranchID, mStatus MergeStatus) (
		*RootMetadataSigned, error)

	// GetRange returns a range of (signed/encrypted) metadata objects
	// corresponding to the passed revision numbers (inclusive).
	GetRange(ctx context.Context, id tlf.TlfID, bid BranchID, mStatus MergeStatus,
	...
	// notifications.
	RegisterForUpdate(ctx context.Context, id tlf.TlfID,
...
type mdServerLocal interface {
	...
	getCurrentMergedHeadRevision(ctx context.Context, id tlf.TlfID) (
...
type BlockServer interface {
	...
	// block.
	Get(ctx context.Context, tlfID tlf.TlfID, id BlockID, context BlockContext) (
	...
	// the error.
	Put(ctx context.Context, tlfID tlf.TlfID, id BlockID, context BlockContext,
	...
	// the caller can treat it as informational and otherwise ignore
	// the error.
	AddBlockReference(ctx context.Context, tlfID tlf.TlfID, id BlockID,
	...
	RemoveBlockReferences(ctx context.Context, tlfID tlf.TlfID,
	...
	ArchiveBlockReferences(ctx context.Context, tlfID tlf.TlfID,
...
type blockServerLocal interface {
	...
	// used during testing.
	getAll(ctx context.Context, tlfID tlf.TlfID) (
...
type BareRootMetadata interface {
	// tlf.TlfID returns the ID of the TLF this BareRootMetadata is for.
	TlfID() tlf.TlfID
...
type MutableBareRootMetadata interface {
	...
	// the given tlf.TlfID and BareTlfHandle. Note that if the given ID/handle are private,
	// rekeying must be done separately.
	Update(tlf tlf.TlfID, h BareTlfHandle) error
